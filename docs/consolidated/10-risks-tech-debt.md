...
> entry required; handled by `14-phase0-assessment.md §7.5 Case 6`. D-1 was pending final schema
...
| R-4 | **Gmail rate limits during fetch phase** — bulk fetch of up to the configured bounded item batch may trigger Gmail 429 | **MEDIUM** | Transition affected items to `RETRY_WAIT` with bounded exponential backoff and `next_retry_at`; persist and publish the QStash continuation. Exhausted retries become `PERMANENTLY_FAILED`; sanitize stored errors. |
| R-5 | **Email metadata growth** — repeated scans retain one durable source row per Gmail message plus per-scan membership rows | **LOW** | Enforce source and membership uniqueness, index tenant/account queries, monitor storage, and use only the approved erasure workflow for deletion. Phase 1A does not store body content or `body_hash`. |
...
| R-9 | **[RESOLVED 2026-07-26] Historical clean-replay ordering defect** — three bridge migrations preserve both historical checksums, bootstrap only during clean replay, act as validation-only no-ops on already-migrated databases, normalize final state, and fail closed on mismatch | **RESOLVED** | Empty 18-migration deploy, production-like out-of-order reconciliation, negative checksum test, and `--from-migrations` replay passed |
| R-10 | **[RESOLVED 2026-07-26] Pre-existing Prisma schema drift** — `20260726010000_reconcile_llm_schema_drift` adds the three nullable LLM columns with exact fingerprint validation and no backfill | **RESOLVED** | Live and migration-directory Prisma diffs return no difference; representative rollback/reapply passed |
| R-11 | **[RESOLVED 2026-07-26] Production dependency advisories** — patched Auth.js, Next.js, Prisma, PostCSS, sharp, find-my-way, and Valibot versions reduce the production audit from 11 vulnerabilities (including 2 critical) to zero | **RESOLVED** | Focused authentication tests, all 248 unit tests, production build, Prisma validation/generation, migration status, and drift checks pass. Residual development-only brace-expansion findings remain in ESLint/Jest tooling and are not present in the production tree. |
